...
<div class="space-y-8 max-w-7xl mx-auto font-sans pb-16 text-gray-900">
        ...
        <div class="flex items-center gap-5 relative z-10">
            ...
            <div class="space-y-1.5">
                <div class="inline-flex items-center gap-2 rounded-md px-2.5 py-1 text-[10px] uppercase tracking-[0.2em] font-mono font-bold bg-surface text-void border border-black/10 mb-1">
                    ...
                    <span>MANAJEMEN GURU KELAS</span>
                </div>
                ...
                <p class="text-xs text-gray-500 font-mono flex flex-wrap items-center gap-3 pt-0.5 uppercase">
                    <span>TINGKAT: <strong class="text-void font-black">KELAS {{ $kelas->tingkat_kelas }}</strong></span>
                    <span>•</span>
                    <span>TOTAL <strong class="text-void font-black">{{ $guruKelas->count() }}</strong> GURU TERDAFTAR</span>
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl p-8 sm:p-10 border border-black/10 shadow-xs space-y-6">
        <div class="flex items-center gap-3.5 pb-5 border-b border-gray-100">
            ...
            <div>
                <h3 class="text-base font-black text-void uppercase tracking-tight font-sans">TAMBAH GURU KE KELAS INI</h3>
                <p class="text-xs text-gray-400 font-mono">Pilih guru mata pelajaran dari daftar guru yang telah terdaftar di sistem</p>
            </div>
        </div>

        <form action="{{ route('admin.kelas.guru.store', $kelas->id_kelas) }}" method="POST" class="flex flex-col sm:flex-row items-center gap-4 font-mono text-xs">
            @csrf
            ...
            <div class="relative w-full flex-1">
                <select name="id_guru_mapel" required
                    ...
                    @foreach($availableGuru as $g)
                        <option value="{{ $g->id }}">
                            {{ strtoupper($g->guru->nama_lengkap ?? '-') }} — {{ strtoupper($g->mapel->nama_mapel ?? '-') }}
                        </option>
                    @endforeach
                </select>
        ...
        @if($availableGuru->isEmpty())
            <p class="text-xs text-gray-400 font-mono font-bold uppercase pt-2">
                <i class="bi bi-info-circle text-cobalt mr-1"></i>
                Semua guru mapel sudah terdaftar di kelas ini, atau belum ada guru mapel yang dibuat.
                <a href="{{ route('admin.mapel.index') }}" class="underline text-cobalt">Kelola Guru Mapel →</a>
            </p>
        @endif
    </div>
    ...
    <div class="bg-white rounded-3xl p-8 border border-black/10 shadow-xs overflow-hidden space-y-6">
        <div class="flex items-center justify-between px-2 font-mono">
            <h3 class="text-sm font-black text-void uppercase tracking-wider font-sans">DAFTAR GURU PENGAJAR DI KELAS INI</h3>
            <span class="px-3 py-1 rounded-lg bg-gray-100 text-void font-bold text-xs">{{ $guruKelas->count() }} GURU</span>
        </div>
            ...
            <table class="w-full text-left font-mono">
                <thead>
                    <tr class="border-b border-gray-200 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                        ...
                        <th class="py-3.5">MATA PELAJARAN</th>
                        <th class="py-3.5 pr-3 text-right">AKSI</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 text-xs font-bold text-gray-800">
                    @forelse($guruKelas as $i => $item)
                    <tr class="hover:bg-gray-50/80 transition-colors group">
                        <td class="py-4 pl-3 text-center text-gray-400">{{ $i + 1 }}</td>
                        <td class="py-4">
                            <div class="flex items-center gap-3.5">
                                <div class="w-9 h-9 rounded-xl bg-void text-white flex items-center justify-center font-black text-xs shrink-0">
                                    {{ substr(preg_replace('/[^A-Za-z]/', '', $item->guruMapel->guru->nama_lengkap ?? 'G'), 0, 2) }}
                                </div>
                                <span class="font-black text-void group-hover:text-cobalt transition-colors text-base font-sans uppercase">
                                    {{ $item->guruMapel->guru->nama_lengkap ?? '-' }}
                                </span>
                            </div>
                        </td>
                        <td class="py-4 text-gray-500 uppercase">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-gray-100 text-void font-bold text-[11px]">
                                <i class="bi bi-book-fill text-cobalt"></i>
                                {{ $item->guruMapel->mapel->nama_mapel ?? '-' }}
                            </span>
                        </td>
                        <td class="py-4 pr-3 text-right">
                            <form action="{{ route('admin.kelas.guru.destroy', $kelas->id_kelas) }}"
                                  method="POST"
                                  onsubmit="return confirm('Hapus {{ $item->guruMapel->guru->nama_lengkap ?? '' }} dari kelas ini?')">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h4 class="font-bold text-void text-sm uppercase">BELUM ADA GURU YANG DITAMBAHKAN KE KELAS INI</h4>
                                <p class="text-xs text-gray-400 font-mono mt-1">Tambahkan guru mata pelajaran menggunakan form di atas</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
