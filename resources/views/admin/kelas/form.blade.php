<div class="space-y-8 max-w-5xl mx-auto font-sans pb-16 text-slate-800">
    ...
    <div class="bg-white rounded-[2.2rem] p-8 sm:p-10 shadow-[0_12px_45px_-10px_rgba(0,0,0,0.06)] relative overflow-hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 ring-1 ring-slate-900/[0.03] transition-all duration-500 hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.08)]">
        ...
        <div class="flex items-center gap-5 relative z-10">
            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-600 text-white flex items-center justify-center shadow-lg shadow-emerald-500/25 shrink-0 transition-transform duration-300 hover:scale-105 hover:rotate-3">
                <i class="bi {{ isset($kelas) ? 'bi-door-open-fill' : 'bi-plus-square-dotted' }} text-3xl"></i>
            </div>
            <div class="space-y-1.5">
                ...
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
                    {{ $title ?? (isset($kelas) ? 'Edit Data Kelas' : 'Tambah Kelas Baru') }}
                </h1>
    ...
    <form action="{{ isset($kelas) ? route('admin.kelas.update', $kelas->id_kelas ?? $kelas->id) : route('admin.kelas.store') }}"
        ...
        {{-- SECTION 1: Rincian Kelas (Floating Elevation) --}}
        <div class="bg-white rounded-[2.2rem] p-8 sm:p-10 shadow-[0_12px_45px_-10px_rgba(0,0,0,0.06)] ring-1 ring-slate-900/[0.03] space-y-6">
            ...
            <div class="flex items-center gap-3.5 pb-5 border-b border-slate-100">
                ...
                <div>
                    <h3 class="text-base font-black text-slate-900 tracking-tight">Rincian Rombongan Belajar</h3>
                    <p class="text-xs text-slate-400 font-semibold">Tentukan tingkat jenjang pendidikan dan penamaan kelas</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- TINGKAT KELAS --}}
                <div>
                    <label class="block text-xs font-black text-slate-700 uppercase tracking-wider mb-2">Tingkat Kelas <span class="text-rose-500">*</span></label>
                    <select name="tingkat_kelas" required
                            class="w-full bg-slate-50 hover:bg-slate-100/70 border border-slate-200/80 rounded-2xl px-4 py-3.5 text-xs font-bold text-slate-900 focus:bg-white focus:outline-none focus:ring-4 focus:ring-emerald-500/15 focus:border-emerald-600 transition-all shadow-inner">
                ...
                {{-- NAMA KELAS --}}
                <div>
                    <label class="block text-xs font-black text-slate-700 uppercase tracking-wider mb-2">Nama Kelas <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama_kelas" required
                           value="{{ old('nama_kelas', $kelas->nama_kelas ?? '') }}"
                           placeholder="Contoh: Kelas 1 A / Kelas 6 Unggulan"
                           class="w-full bg-slate-50 hover:bg-slate-100/70 border border-slate-200/80 rounded-2xl px-4 py-3.5 text-xs font-bold text-slate-900 placeholder:text-slate-400 placeholder:font-medium focus:bg-white focus:outline-none focus:ring-4 focus:ring-emerald-500/15 focus:border-emerald-600 transition-all shadow-inner">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.2rem] p-6 sm:p-8 shadow-[0_12px_45px_-10px_rgba(0,0,0,0.06)] ring-1 ring-slate-900/[0.03] flex flex-col sm:flex-row items-center justify-between gap-4">
            <span class="text-xs text-slate-400 font-bold flex items-center gap-2">
                <div class="w-6 h-6 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center font-black">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto justify-end">
                ...
                <button type="submit"
                        class="group px-8 py-3.5 rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold text-xs shadow-lg shadow-emerald-500/25 transition-all duration-300 active:scale-[0.98] text-center w-full sm:w-auto inline-flex items-center justify-center gap-3">
