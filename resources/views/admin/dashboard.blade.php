...
<div class="space-y-8 max-w-[1500px] w-full mx-auto font-sans pb-16" x-data="{ rooms: {{ json_encode($roomsData) }} }">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div class="lg:col-span-5 bg-void text-white rounded-3xl p-8 shadow-xl flex flex-col justify-between border border-black">
            <div class="flex items-center justify-between">
                <span class="text-xs font-mono font-bold uppercase tracking-[0.2em] text-gray-400">Sistem Informasi Akademik</span>
                <span class="w-3 h-3 rounded-full bg-signal animate-pulse" title="Live System"></span>
            </div>

            <div class="my-6">
                <div class="flex items-baseline gap-2">
                    <span class="text-6xl font-black font-mono tracking-tighter text-white">{{ number_format($totalSiswa ?? 364) }}</span>
                ...
                <p class="text-xs font-mono text-gray-400 mt-3 uppercase tracking-wider leading-relaxed">
                    SDN 01 DURIAN GADANG • TAHUN AJARAN {{ $taAktif->tahun_ajaran ?? '2025/2026' }} ({{ strtoupper($taAktif->semester ?? 'GANJIL') }})
                </p>
            </div>

            <div class="pt-6 border-t border-white/20 flex flex-wrap items-center justify-between gap-4">
                <span class="text-xs font-mono text-white font-bold">{{ count($distribusiKelas ?? []) ?: 6 }} ROMBEL AKTIF</span>
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.siswa.index') }}" class="px-4 py-2 rounded-xl bg-white text-void font-bold text-xs hover:bg-signal hover:text-white transition-colors inline-flex items-center gap-2">
        ...
        <div class="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-3xl p-8 border border-black/10 shadow-xs flex flex-col justify-between">
                ...
                <div class="my-4">
                    <span class="text-5xl font-black font-mono tracking-tight text-void">{{ number_format($totalGuru ?? 24) }}</span>
            ...
            <div class="bg-white rounded-3xl p-8 border border-black/10 shadow-xs flex flex-col justify-between">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-mono font-bold uppercase tracking-widest text-gray-400">Kurikulum Merdeka</span>
                    <i class="bi bi-book-half text-void text-xl"></i>
                </div>
                <div class="my-4 flex items-baseline gap-6">
                    <div>
                        <span class="text-5xl font-black font-mono tracking-tight text-void">{{ number_format($totalMapel ?? 12) }}</span>
                        <span class="text-sm font-bold text-gray-400 ml-1">Mapel</span>
                    </div>
                    <div>
                        <span class="text-3xl font-black font-mono tracking-tight text-gray-700">{{ number_format($totalEkskul ?? 5) }}</span>
    ...
    {{-- PIXEL SEAT MATRIX (REALTIME WIDGET) --}}
    <div class="bg-white rounded-3xl p-8 border border-black/10 shadow-xs space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-100">
            <div>
                ...
                <p class="text-xs font-mono text-gray-500 mt-1">Setiap kotak kecil mewakili 1 siswa (kapasitas 32 bangku per kelas). Sinkron langsung dengan distribusi rombel.</p>
            </div>
            <div class="flex items-center gap-4 font-mono text-xs font-bold">
                <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-xs bg-emerald-600 inline-block"></span> Terisi</span>
                <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-xs bg-amber-500 inline-block"></span> Cadangan</span>
                <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded-xs bg-signal inline-block"></span> Kosong</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <template x-for="room in rooms" :key="room.code">
                <div class="p-6 rounded-2xl bg-gray-50 border border-black/10 hover:border-void transition-all duration-300 flex flex-col justify-between space-y-5">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] font-mono text-gray-500 truncate max-w-[160px]" x-text="room.teacher"></p>
                            </div>
                        </div>
                    ...
                    <div class="p-3.5 rounded-xl bg-white border border-black/5 space-y-2">
                        <div class="text-[10px] font-mono text-gray-400 uppercase flex justify-between font-bold">
                            <span>Kapasitas Bangku (32)</span>
                            <span x-text="room.present + ' Siswa'"></span>
                        </div>
                        <div class="grid grid-cols-8 gap-1.5">
                            <!-- Present seats -->
                            <template x-for="i in room.present" :key="'p'+i">
                                <div class="w-full h-3 rounded-xs bg-emerald-600 hover:scale-125 transition-transform cursor-pointer" :title="'Siswa #' + i"></div>
                            </template>
                            <!-- Sick/Reserved seats -->
                            <template x-for="j in room.sick" :key="'s'+j">
                                <div class="w-full h-3 rounded-xs bg-amber-500 hover:scale-125 transition-transform cursor-pointer" title="Bangku Cadangan"></div>
                            </template>
                            <!-- Alpha/Empty seats -->
                            <template x-for="k in room.alpha" :key="'a'+k">
                                <div class="w-full h-3 rounded-xs bg-signal hover:scale-125 transition-transform cursor-pointer" title="Bangku Kosong"></div>
                            </template>
                        </div>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 bg-white rounded-3xl p-8 border border-black/10 shadow-xs">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-100 mb-6">
                <div>
                    <span class="text-xs font-mono font-bold uppercase tracking-widest text-gray-400">Tabel Distribusi Rombel</span>
                            ...
                            @foreach($distribusiKelas as $dk)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="py-4 pl-3 font-extrabold text-void flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-lg bg-void text-white flex items-center justify-center font-bold text-xs shrink-0">
                                            {{ preg_replace('/[^0-9]/', '', $dk->kelas->nama_kelas ?? '1') ?: 'SD' }}
                                        </div>
                                        <span class="text-sm font-bold">{{ strtoupper($dk->kelas->nama_kelas ?? 'KELAS') }}</span>
                                    </td>
                                    ...
                                    <td class="py-4 pr-3 w-40">
                                        <div class="flex items-center gap-3 justify-end">
                                            <div class="w-20 bg-gray-100 h-2.5 rounded-full overflow-hidden">
                                                <div class="{{ $barColor }} h-full transition-all duration-500" style="width: {{ $persen }}%"></div>
                                            </div>
            ...
            @else
                <div class="py-16 text-center font-mono">
                    <div class="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center mx-auto mb-3 text-gray-400">
        ...
        <div class="lg:col-span-4 space-y-6 font-mono">
            ...
            <div class="bg-white rounded-3xl p-8 border border-black/10 shadow-xs space-y-5">
                <div class="flex items-center justify-between pb-4 border-b border-gray-100">
                    <div>
                        <span class="block text-xs font-bold uppercase tracking-widest text-gray-400">Profil Sekolah</span>
