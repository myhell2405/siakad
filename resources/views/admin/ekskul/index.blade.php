...
<div class="space-y-8 max-w-7xl mx-auto font-sans pb-16 text-slate-800">
        ...
        <div class="flex items-center gap-5 relative z-10">
            ...
            <div class="space-y-1.5">
                ...
                <p class="text-xs text-slate-400 font-bold flex items-center gap-2 pt-0.5">
                    ...
                    <span>Total <strong class="text-slate-700">{{ $ekskul->total() }}</strong> Ekskul Terdaftar</span>
                </p>
            </div>
        </div>

        <a href="{{ route('admin.ekskul.create') }}"
           class="group relative z-10 inline-flex items-center gap-4 pl-6 pr-2 py-2 rounded-full bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white font-bold text-xs shadow-lg shadow-violet-500/25 hover:shadow-xl hover:shadow-violet-500/30 transition-all duration-300 active:scale-[0.98] w-full sm:w-auto justify-between sm:justify-start">
            <span class="tracking-wide">Tambah Ekskul Baru</span>
    ...
    <div class="bg-white rounded-[2rem] p-4 sm:p-6 shadow-[0_10px_35px_-10px_rgba(0,0,0,0.04)] ring-1 ring-slate-900/[0.03]">
        <form action="{{ route('admin.ekskul.index') }}" method="GET" class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="relative w-full sm:w-96">
                <input type="text" name="search" value="{{ request('search') }}"
                       placeholder="Cari nama ekskul atau keterangan..."
                       class="w-full pl-11 pr-4 py-3 bg-slate-50 hover:bg-slate-100/70 focus:bg-white border border-slate-200/80 rounded-2xl text-xs font-bold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-violet-500/15 focus:border-violet-600 transition-all shadow-inner">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th class="pb-4">Nama Ekstrakurikuler</th>
                        <th class="pb-4">Guru Pembina</th>
                        <th class="pb-4">Keterangan / Jadwal</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs font-bold text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        <td class="py-4 pl-4 text-slate-400 font-semibold">
                            {{ $ekskul->firstItem() + $loop->index }}
                        </td>
                        <td class="py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center font-black text-sm shrink-0 group-hover:scale-110 transition-transform">
                                    <i class="bi bi-award"></i>
                                </div>
                                <span class="text-sm font-black text-slate-900">{{ $item->nama_ekskul }}</span>
                            </div>
                        </td>
                        <td class="py-4">
                            @if($item->guru)
                                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 text-slate-800 text-[11px] font-bold">
                                    <i class="bi bi-person-fill text-violet-600"></i>
                        ...
                        <td class="py-4 text-slate-500 font-medium max-w-xs truncate">
                            {{ $item->keterangan ?? '-' }}
                        </td>
                        <td class="py-4 pr-4 text-right">
                            <div class="inline-flex items-center justify-end gap-1.5">
                                ...
                                {{-- DELETE BUTTON --}}
                                <form action="{{ route('admin.ekskul.destroy', $item->id_ekskul) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus ekstrakurikuler ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded-xl bg-slate-100 hover:bg-gradient-to-r hover:from-rose-600 hover:to-red-600 text-slate-700 hover:text-white flex items-center justify-center transition-all shadow-2xs hover:shadow-md hover:shadow-rose-500/20 active:scale-95" title="Hapus Ekskul">
                    ...
                    <tr>
                        <td colspan="5" class="py-12 text-center text-slate-400 font-medium">
                            <div class="flex flex-col items-center justify-center gap-3">
                                ...
                                <span>Data ekstrakurikuler belum tersedia atau tidak ditemukan.</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
