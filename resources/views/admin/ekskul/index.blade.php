...
<div class="space-y-8 max-w-7xl mx-auto font-sans pb-16 text-gray-900">
    ...
    <div class="bg-white rounded-3xl p-8 sm:p-10 border border-black/10 shadow-xs relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div class="flex items-center gap-5 relative z-10">
            <div class="w-16 h-16 rounded-2xl bg-void text-white flex items-center justify-center border border-black shadow-md shrink-0">
                <i class="bi bi-trophy-fill text-3xl text-signal"></i>
            </div>
            <div class="space-y-1.5">
                ...
                <h1 class="text-3xl sm:text-4xl font-black text-void tracking-tight uppercase">
                    {{ strtoupper($title ?? 'DATA EKSTRAKURIKULER') }}
                </h1>
                <p class="text-xs text-gray-500 font-mono flex items-center gap-2 pt-0.5 uppercase">
                    <span>SDN 01 DURIAN GADANG</span>
                    <span>•</span>
                    <span>TOTAL <strong class="text-void font-black">{{ $ekskul->total() }}</strong> EKSKUL TERDAFTAR</span>
                </p>
            </div>
        </div>
        ...
        <a href="{{ route('admin.ekskul.create') }}"
           class="inline-flex items-center gap-3 px-6 py-3.5 rounded-xl bg-void hover:bg-black text-white font-mono font-bold text-xs transition shadow-md uppercase">
            <i class="bi bi-plus-lg text-signal"></i> TAMBAH EKSKUL BARU
        </a>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-black/10 shadow-xs overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface border-b border-black/10 text-void font-mono text-[10px] uppercase tracking-wider">
                        ...
                        <th class="py-4">NAMA EKSTRAKURIKULER</th>
                        <th class="py-4">GURU PEMBINA</th>
                        <th class="py-4">KETERANGAN / JADWAL</th>
                        ...
                        <td class="py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-xl bg-void text-white flex items-center justify-center font-bold text-sm shrink-0 shadow-xs">
                                    <i class="bi bi-award text-signal"></i>
                                </div>
                                <span class="text-sm font-bold text-void uppercase">{{ $item->nama_ekskul }}</span>
                            </div>
                        </td>
                        <td class="py-4">
                            @if($item->guru)
                                <div class="inline-flex items-center gap-2 px-3 py-1 rounded bg-gray-100 text-void border border-black/5 text-[11px] font-mono font-bold uppercase">
                                    <i class="bi bi-person-fill text-cobalt"></i>
                                    <span>{{ $item->guru->nama_lengkap }}</span>
                                </div>
                            @else
                                <span class="text-gray-400 italic font-normal uppercase font-mono">BELUM DITENTUKAN</span>
                            @endif
                        </td>
                        <td class="py-4 text-gray-600 font-medium uppercase max-w-xs truncate">
                            {{ $item->keterangan ?? '-' }}
                        </td>
                        <td class="py-4 pr-6 text-right">
                            <div class="inline-flex items-center justify-end gap-1.5">
                                ...
                                <form action="{{ route('admin.ekskul.destroy', $item->id_ekskul) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus ekstrakurikuler ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded-lg bg-gray-100 hover:bg-signal text-void hover:text-white flex items-center justify-center transition" title="Hapus Ekskul">
                    ...
                    <tr>
                        <td colspan="5" class="py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h4 class="font-bold text-void text-base uppercase font-mono">BELUM ADA DATA EKSTRAKURIKULER</h4>
                                <p class="text-xs text-gray-500 mt-1 max-w-sm">Daftar kegiatan ekskul masih kosong. Klik tombol Tambah Ekskul Baru di atas untuk mulai memasukkan data.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        @if($ekskul->hasPages())
            {{ $ekskul->links('components.pagination') }}
        @endif
    </div>
